    ...
    try:
        ...
        if response.status_code in [200, 201, 204]:
            ...
            if response.status_code == 404:
                st.error("❌ user_credentials 表不存在，请在 Supabase 控制台创建表")
...
def show_user_info():
    ...
    if username and user_info:
        ...
        st.sidebar.markdown(f"**注册时间：** {created_at[:10]}")
        st.sidebar.markdown(f"**最后登录：** {last_login[:19] if last_login != '从未登录' else '从未登录'}")
